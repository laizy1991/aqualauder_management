    ...
    var Status = {
        ...
        initStatusPostForm: function() {
            ...
            $('#btnSubmit').click(function(){
                ...
                if (startTime == "") {
                    $("#startTime").parent().find(".error-msg").text("请选择执行时间");
                    return false;
                } else {
                    $("#startTime").parent().find(".error-msg").text("");
                }
                var $form=$("#statusPostForm");
                ajax.post($form.attr('action'), $form.serialize(), function(result){
                    if(result.success){
                        dd.alert('添加定时动态任务成功！', function(){
                        ...
                        window.location.href="/status/list4Task?page=1"
                    }else{
